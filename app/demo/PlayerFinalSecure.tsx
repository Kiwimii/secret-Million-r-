...
import { PHASE_LABELS, ROUNDS, getRoundPhaseSequence } from "@/lib/game/constants";
...
import { getPlayerStatusClass, getPlayerStatusLabel } from "@/lib/demo/useDemoGame";

function phaseInstruction(phase: string, isMillionaire: boolean): string {
  const instructions: Record<string, string> = {
    lobby: "Warte, bis André den Startmillionär ausgelost und die erste Runde freigegeben hat.",
    ...
    discussion: "Diskutiert offen. Der Fragesteller darf über seine Antwort berichten oder bluffen.",
    ...
    evaluation: "Die Spielleitung zählt reguläre Stimmen und mögliche Vorteilskorrekturen.",
    role_transfer: "Gib deine geheime Rollenentscheidung ab. Niemand bestimmt selbst den Nachfolger.",
    finished: "Das Spiel ist beendet. Der Hauptgewinner wird aufgelöst.",
  };
  return instructions[phase] ?? "Warte auf die nächste Freigabe der Spielleitung.";
}

export default function PlayerFinalSecure({
  snapshot,
  player,
  roleRevealed,
  voteTargetId,
  ...
  const isMillionaire = player.id === snapshot.game.millionairePlayerId && player.role === "millionaire";
  const hasActiveRole = player.role !== "none" && capabilities.eligibleToWin;
  const roleTransferResolved = Boolean(snapshot.roleTransferResolvedByRound[round]);
  const nextRoleLocked = snapshot.game.phase === "role_transfer" && roleTransferResolved;
  ...
  const roundVotes = snapshot.votesByRound[round] ?? { main: {}, runoff: {} };
  const submittedTargetId = roundVotes[voteStage][player.id];
  const submittedTarget = snapshot.game.players.find((entry) => entry.id === submittedTargetId);
  const allowedCandidates = voteStage === "runoff"
    ? (snapshot.runoffCandidateIdsByRound[round] ?? [])
        .map((id) => snapshot.game.players.find((entry) => entry.id === id))
  ...
  const roleDecision = snapshot.roleDecisionsByRound[round]?.[player.id];
  const phaseSequence = getRoundPhaseSequence(round);
  const phaseIndex = snapshot.game.phase === "lobby"
    ? 0
    : Math.max(0, phaseSequence.indexOf(snapshot.game.phase));
  const progress = snapshot.game.phase === "finished"
    ? 100
    : Math.round((phaseIndex / Math.max(1, phaseSequence.length - 1)) * 100);
  const winner = snapshot.finalWinner?.winnerPlayerId
    ? snapshot.game.players.find((entry) => entry.id === snapshot.finalWinner?.winnerPlayerId)
    : undefined;
  ...
  return (
    <section className="demo-player-screen">
      ...
      <div className="demo-player-top">
        <div>
          <p className="section-label">Runde {round} · {roundDefinition.timing}</p>
      ...
      <div className="demo-progress-track" aria-label={`Spielfortschritt ${progress} Prozent`}>
      ...
      <div className="demo-player-content">
        <article className={`demo-role-card ${roleRevealed ? "revealed" : ""} ${isMillionaire ? "millionaire" : "investigator"}`}>
          {nextRoleLocked ? (
            <div className="demo-role-hidden">
              ...
              <p>Die Zufallsauslosung ist abgeschlossen. Deine Rolle wird erst beim Start der nächsten Runde sichtbar.</p>
            </div>
          ) : !roleRevealed ? (
            <div className="demo-role-hidden">
              <span className="demo-lock">◆</span>
              ...
              <button className="button button-primary" type="button" onClick={onRevealRole}>
                Rolle aufdecken
              </button>
            </div>
          ) : !hasActiveRole ? (
            <div className="demo-role-revealed">
              ...
              <p>Du bleibst bei Anwesenheit Teil von Diskussionen, Challenges und Abstimmungen, kannst aber nicht mehr Millionär oder Hauptgewinner werden.</p>
            </div>
          ) : (
            <div className="demo-role-revealed">
              ...
              <p>{isMillionaire
                ...
                : "Beobachte die Gruppe, nutze Informationen und stimme gegen deinen Verdächtigen."}</p>
            </div>
          )}
        </article>
        ...
        {isMillionaire && roleRevealed && !nextRoleLocked && missionStatus !== "unassigned" && (
          <article className="demo-panel demo-mission-card">
            <p className="section-label">Geheime Mission · nur für dich</p>
            <h2>{mission?.title}</h2>
            <p>{mission?.task}</p>
            <dl>
              <div><dt>Erfolg</dt><dd>{mission?.successCriteria}</dd></div>
              <div><dt>Zeitfenster</dt><dd>{mission?.timeWindow}</dd></div>
            </dl>
            <span className={`mission-state mission-${missionStatus}`}>{missionStatus}</span>
          </article>
        )}
        ...
        {isQuestioner && questionText && snapshot.game.phase !== "lobby" && (
          <article className="demo-panel demo-question-card">
            ...
            <h2>{questionText}</h2>
            <div className={`demo-secret-answer ${questionAnswer ? "yes" : "no"}`}>
              {questionAnswer ? "JA" : "NEIN"}
            </div>
            <p>Du darfst der Gruppe die Wahrheit sagen oder bluffen. Zeige diese Karte nicht herum.</p>
          </article>
        )}

        {snapshot.game.phase === "voting" && capabilities.canVote && (
          <article className="demo-panel demo-vote-card">
            <p className="section-label">{voteStage === "runoff" ? "Geheime Stichwahl" : "Geheime Hauptabstimmung"}</p>
            <h2>{voteStage === "runoff" ? "Wer scheidet im Gleichstand aus?" : "Wer ist der Millionär?"}</h2>
            {submittedTarget ? (
              <div className="demo-vote-confirmed">
                <span>✓</span><strong>Stimme gespeichert</strong><p>Deine Auswahl: {submittedTarget.name}</p>
              </div>
            ) : (
              <>
                <div className="demo-vote-grid">
                  {allowedCandidates.map((candidate) => (
                    <button className={voteTargetId === candidate.id ? "selected" : ""} type="button" key={candidate.id} onClick={() => onVoteTarget(candidate.id)}>
                      <span>{candidate.name.slice(0, 1)}</span><strong>{candidate.name}</strong>
                    </button>
                  ))}
                </div>
        ...
        {snapshot.game.phase === "role_transfer" && capabilities.eligibleToWin && !roleTransferResolved && (
          <article className="demo-panel demo-role-decision-card">
            <p className="section-label">Geheime Rollenentscheidung</p>
            <h2>{isMillionaire ? "Was geschieht mit dem Korken?" : "Bestätige deine Rollenkarte."}</h2>
            {roleDecision ? (
              <div className="demo-vote-confirmed"><span>✓</span><strong>Entscheidung gespeichert</strong><p>Die Auflösung erfolgt erst durch die Spielleitung.</p></div>
            ) : isMillionaire ? (
              <div className="demo-role-decision-actions">
                <button className="button button-secondary full-width" type="button" onClick={() => onRoleDecision("keep")}>Korken behalten</button>
                <button className="button button-primary full-width" type="button" onClick={() => onRoleDecision("release")}>Korken abgeben</button>
                <p>Bei Abgabe bestimmst du keinen Nachfolger. Die App lost geheim und zufällig aus allen anderen Berechtigten aus.</p>
              </div>
            ) : (
              <button className="button button-primary full-width" type="button" onClick={() => onRoleDecision("not_millionaire")}>Bestätigen: Ich bin nicht der Millionär</button>
            )}
          </article>
        )}
        ...
        {snapshot.game.phase === "result" && snapshot.lastOutcome && (
          <article className="demo-panel demo-result-card">
            ...
            <h2>{snapshot.game.players.find((entry) => entry.id === snapshot.lastOutcome?.eliminatedPlayerId)?.name} scheidet aus dem Gewinnerpool aus.</h2>
            <p>{snapshot.lastOutcome.millionaireSurvived
              ? "Der Millionär blieb unerkannt."
              : "Der Millionär wurde enttarnt. Vor der nächsten Runde wird zufällig neu ausgelost."}</p>
          </article>
        )}
        ...
        {snapshot.game.phase === "finished" && winner && (
          <article className="demo-panel demo-result-card demo-winner-card">
            ...
            <p>Es gibt genau einen Hauptgewinner. Der Entscheidungsweg war: {snapshot.finalWinner?.reason}.</p>
          </article>
        )}

        {snapshot.game.phase !== "voting" && snapshot.game.phase !== "result" && snapshot.game.phase !== "finished" && snapshot.game.phase !== "role_transfer" && (
