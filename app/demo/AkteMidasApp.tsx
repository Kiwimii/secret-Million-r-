    ...
    <DashboardShell view={view} controller={controller}>
      ...
      <div className={styles.hostGrid}>
        ...
        <aside className={styles.sideColumn}>
          ...
          <Card id="log" eyebrow="Leitstellenarchiv" title="Ereignisprotokoll"><EventList events={view.notifications} view={view} /></Card>
        </aside>
      </div>
...
function PlayerDashboard({ view, controller }: { view: MetaGameView; controller: ReturnType<typeof useMetaGame> }) {
  ...
  return <DashboardShell view={view} controller={controller}>
    ...
    <div className={styles.playerGrid}>
      ...
      {roleDecisionPending && roleDecisionDismissed && <Card eyebrow="Nächste Deckung" title="Rollenentscheidung ausstehend"><p>Die nächste Runde wartet auf Ihre private Entscheidung.</p><button className={styles.primaryButton} onClick={() => setRoleDecisionDismissed(false)}>Entscheidung öffnen</button></Card>}
      <Card id="log" eyebrow="Einsatzakte" title="Persönliches Protokoll"><EventList events={view.notifications} view={view} />{(view.personalHistory ?? []).length > 0 && <div className={styles.historyTable}>{view.personalHistory?.map((entry) => <div key={entry.roundNumber}><b>Runde {entry.roundNumber}</b><span>{entry.role === "millionaire" ? "Millionär" : entry.role === "investigator" ? "Ermittler" : "Außer Wertung"}</span><span>{entry.voteTargetId ? `Stimme: ${getName(view,entry.voteTargetId)}` : "Keine Stimme"}</span><strong>{entry.pointsAwarded >= 0 ? "+" : ""}{entry.pointsAwarded} Punkte</strong></div>)}</div>}</Card>
    </div>
...
function RoleDecisionOverlay({ canKeep, busy, onClose, onDecision }: { canKeep: boolean; busy: boolean; onClose(): void; onDecision(decision: "keep" | "transfer"): void }) {
