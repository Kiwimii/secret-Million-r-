      ...
      <div className="demo-player-content">
        <article className={`demo-role-card ${roleRevealed ? "revealed" : ""} ${isMillionaire ? "millionaire" : "investigator"}`}>
          {!roleRevealed ? (
            ...
            <div className="demo-role-revealed">
              ...
              <p>
                Du bleibst bei Anwesenheit Teil von Diskussionen, Challenges und Abstimmungen,
                kannst aber nicht mehr Millionär oder Hauptgewinner werden.
              </p>
            </div>
          ) : (
            <div className="demo-role-revealed">
        ...
        {!capabilities.eligibleToWin && player.attendanceStatus !== "departed" && (
          <article className="demo-panel demo-info-card danger">
            ...
            <p>
              Du spielst weiter mit, darfst diskutieren, Challenges bestreiten und abstimmen.
              Den Hauptpreis kannst du nicht mehr gewinnen.
            </p>
          </article>
        )}

        {isMillionaire && roleRevealed && missionStatus !== "unassigned" && (
        ...
        {snapshot.game.phase === "voting" && capabilities.canVote && (
          <article className="demo-panel demo-vote-card">
            <p className="section-label">
              {voteStage === "runoff" ? "Geheime Stichwahl" : "Geheime Hauptabstimmung"}
            </p>
            <h2>
              {voteStage === "runoff"
            ...
            {submittedTarget ? (
              ...
              <>
                ...
                <button className="button button-primary full-width" type="button" onClick={onSubmitVote}>
        ...
        {snapshot.game.phase === "role_transfer" && capabilities.eligibleToWin && (
          <article className="demo-panel demo-role-decision-card">
            ...
            {roleDecision ? (
              ...
              <div className="demo-role-decision-actions">
                <button className="button button-secondary full-width" type="button" onClick={() => onRoleDecision("keep")}>
                  Korken behalten
                </button>
                <button className="button button-primary full-width" type="button" onClick={() => onRoleDecision("release")}>
                  Korken abgeben
                </button>
                <p>
                  Bei Abgabe bestimmst du keinen Nachfolger. Die App lost geheim und zufällig aus
                  allen anderen Berechtigten aus.
                </p>
              </div>
            ) : (
              <button className="button button-primary full-width" type="button" onClick={() => onRoleDecision("not_millionaire")}>
        ...
        {snapshot.game.phase === "result" && snapshot.lastOutcome && (
          <article className="demo-panel demo-result-card">
            ...
            <h2>
              {snapshot.game.players.find((entry) => entry.id === snapshot.lastOutcome?.eliminatedPlayerId)?.name}
