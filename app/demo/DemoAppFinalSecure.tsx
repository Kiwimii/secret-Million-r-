...
export default function DemoAppFinalSecure() {
  ...
  useEffect(() => {
    ...
    else if (savedMode && ["portal", "host", "player"].includes(savedMode)) setMode(savedMode);
  ...
  function handlePrimary() {
    if (snapshot.game.phase === "lobby" && !snapshot.game.millionairePlayerId) {
      run(() => { actions.drawInitialMillionaire(); }, "Der Startmillionär wurde zufällig und geheim ausgelost.");
      return;
    }
  ...
  function handleRandomDraw() {
    if (["result", "role_transfer", "finished"].includes(snapshot.game.phase)) {
      setError("In dieser Phase erfolgt die Neubesetzung ausschließlich über die geheime Rollenentscheidung.");
      return;
    }
    run(() => {
  ...
  function previewEvaluation() {
    run(() => {
      ...
      const names = nextEvaluation.topPlayerIds
        ...
        .filter(Boolean)
        .join(", ");
      setMessage(nextEvaluation.requiresRunoff ? `Gleichstand zwischen: ${names}.` : `Eindeutige Höchstzahl bei: ${names}.`);
    });
  }
    ...
    <main className="demo-app">
      <header className="demo-header">
        <Link href="/" className="demo-brand" aria-label="Zur Startseite">
          ...
          <span><strong>Secret Millionär</strong><small>Blaue Adria · Einsatztest</small></span>
        </Link>
        <div className="demo-connection"><span className="live-dot" />Browser-Sync aktiv</div>
      </header>
      ...
      {mode === "portal" && (
        <PortalFinal
          ...
          onPlayer={(playerId) => { setSelectedPlayerId(playerId); selectMode("player"); }}
      ...
      {mode === "host" && (
        <HostFinal
          ...
          onPendingAction={(playerId, action) => setPendingActions((current) => ({ ...current, [playerId]: action }))}
          onPlayerAction={(playerId) => run(() => {
            const player = snapshot.game.players.find((entry) => entry.id === playerId);
            const action = pendingActions[playerId] ?? "eliminate";
            if (!window.confirm(`${player?.name ?? "Spieler"}: Statusänderung „${action}“ wirklich anwenden?`)) return;
          ...
          onFillRoleDecisions={() => run(actions.fillMissingRoleDecisions, "Fehlende Rollenentscheidungen wurden für den Test ergänzt.")}
          onResolveRoleTransfer={() => run(() => { actions.resolveRoleTransfer(); }, "Die Rollenentscheidung wurde geheim aufgelöst.")}
        />
      )}
      ...
      {mode === "player" && (
        <PlayerFinal
          ...
          onSubmitVote={() => {
            if (!voteTargetId) { setError("Wähle zuerst einen Verdächtigen aus."); return; }
            run(() => actions.submitVote(selectedPlayer.id, voteTargetId), "Deine Stimme wurde geheim gespeichert.");
          }}
          onRoleDecision={(decision) => run(() => actions.submitRoleDecision(selectedPlayer.id, decision), "Deine geheime Rollenentscheidung wurde gespeichert.")}
        />
      )}
      ...
      <footer className="demo-footer">
        <span>Stand {formatUpdatedAt(snapshot.updatedAt)}</span><span>Demo-Code 472918</span>
      </footer>
    </main>
  );
}
