-- Repariert ausschließlich die manuelle Korkenentscheidung durch André.
-- Die bisherige Notfallfunktion setzte nur einen Fortschrittshaken und ließ die
-- Partie deshalb in role_transfer stehen.

create or replace function public.host_rescue_live_role_decision(
  target_game_id uuid,
...
begin
  ...
  if target_member_id is distinct from active_millionaire then
    raise exception 'Nur für den aktuellen Millionär kann die Korkenentscheidung ersetzt werden.';
  end if;
  if not exists (
    select 1
    ...
    raise exception 'Für diesen Millionär ist keine Rollenentscheidung offen.';
  end if;
  ...
  insert into public.role_decisions (
    game_id, after_round, member_id, decision, target_member_id,
    submitted_at, host_confirmed_at, rejected_target_ids
  ) values (
    target_game_id, active_round, target_member_id, decision_value, selected_target,
    now(), case when requested_decision = 'keep' then now() else null end, '{}'::uuid[]
  )
  ...
  values (target_game_id, 'host_rescued_role_decision');
...
revoke all on function public.host_rescue_live_role_decision(uuid, uuid, text) from public;
grant execute on function public.host_rescue_live_role_decision(uuid, uuid, text) to authenticated;
