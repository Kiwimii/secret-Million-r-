  ...
  const actions = useMemo<DemoActions>(
    () => ({
      ...
      drawReplacementMillionaire() {
        if (
          ["lobby", "result", "role_transfer", "finished"].includes(
            snapshot.game.phase,
          )
        ) {
          throw new Error(
      ...
      setMissionStatus(status) {
        if (status === "assigned" && snapshot.game.phase !== "mission") {
          throw new Error(
            "Die Mission kann nur in der Missionsphase ausgegeben werden.",
          );
        }
        if (
          ...
          throw new Error(
            "Erfolg oder Scheitern wird ausschließlich in der Missionsbewertung festgelegt.",
          );
        }
      ...
      setQuestioner(playerId) {
        if (snapshot.game.phase !== "question") {
          throw new Error(
            "Der Fragesteller wird ausschließlich in der Fragephase festgelegt.",
          );
        }
      ...
      setQuestion(question, answer) {
        if (snapshot.game.phase !== "question") {
          throw new Error(
            "Frage und Antwort dürfen nur in der Fragephase gespeichert werden.",
          );
        }
      ...
      setAdvantage(selection) {
        if (snapshot.game.phase !== "advantage") {
          throw new Error(
            "Der Vorteil wird ausschließlich vor Öffnung der Abstimmung festgelegt.",
          );
        }
      ...
      fillMissingVotes() {
        if (snapshot.game.phase !== "voting") {
          throw new Error(
            "Stimmen können nur während einer geöffneten Abstimmung ergänzt werden.",
          );
        }
      ...
      evaluateCurrentVotes() {
        if (snapshot.game.phase !== "evaluation") {
          throw new Error(
            "Die Auswertung ist erst nach Schließen der Abstimmung verfügbar.",
          );
        }
      ...
      startRunoff(candidatePlayerIds) {
        if (snapshot.game.phase !== "evaluation") {
          throw new Error(
            "Eine Stichwahl kann nur aus der Auswertungsphase gestartet werden.",
          );
        }
      ...
      finalizeCurrentRound() {
        if (snapshot.game.phase !== "evaluation") {
          throw new Error(
            "Ein Rundenergebnis kann nur aus der Auswertungsphase veröffentlicht werden.",
          );
        }
